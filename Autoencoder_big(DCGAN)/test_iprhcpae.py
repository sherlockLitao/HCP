# python test_iprhcpae.py --source-data MNIST
# python test_iprhcpae.py --source-data CelebA --landmark-interval 1
import argparse
import pickle
import torch.utils.data
import Methods.iprhcpae as iprhcpae
...
parser = argparse.ArgumentParser(description='IPRHCP Distance Autoencoder Example')
...
if __name__ == '__main__':
    ...
    loss = iprhcpae.train_model(model, src_loaders['train'], src_loaders['val'], device, args)

    # conditional generation
    model.eval()
    evaluation.sampling(model, device, args.epochs, args, prefix='iprhcp-ae', nrow=4)

    # save models and learning results
    model = model.to('cpu')
    torch.save(model.state_dict(),
               '{}/iprhcp-ae_model_{}.pt'.format(args.resultpath, args.source_data))
    with open('{}/iprhcp-ae_loss_{}.pkl'.format(args.resultpath, args.source_data), 'wb') as f:
        pickle.dump(loss, f)
    print('\n')
