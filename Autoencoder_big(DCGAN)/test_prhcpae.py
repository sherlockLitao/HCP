# python test_prhcpae.py --source-data MNIST
# python test_prhcpae.py --source-data CelebA --landmark-interval 1
import argparse
import pickle
import torch.utils.data
import Methods.prhcpae as prhcpae
...
parser = argparse.ArgumentParser(description='PRHCP Distance Autoencoder Example')
...
if __name__ == '__main__':
    ...
    loss = prhcpae.train_model(model, src_loaders['train'], src_loaders['val'], device, args)

    # conditional generation
    model.eval()
    evaluation.sampling(model, device, args.epochs, args, prefix='prhcp-ae', nrow=4)

    # save models and learning results
    model = model.to('cpu')
    torch.save(model.state_dict(),
               '{}/prhcp-ae_model_{}.pt'.format(args.resultpath, args.source_data))
    with open('{}/prhcp-ae_loss_{}.pkl'.format(args.resultpath, args.source_data), 'wb') as f:
        pickle.dump(loss, f)
    print('\n')
