...
from ae.trainer_wae import WAEBatchTrainer
...
def main():
    # train args
    parser = argparse.ArgumentParser(description='Wasserstein Autoencoder PyTorch MNIST Example')
    parser.add_argument('--datadir', default='./data/', help='path to dataset')
    parser.add_argument('--outdir', default='./output/wae/', help='directory to output images and model checkpoints')
    ...
    # create batch wasserstein autoencoder trainer
    trainer = WAEBatchTrainer(model, optimizer, distribution_fn, device=device)
    ...
    for epoch in range(args.epochs):
        ...
        vutils.save_image(batch['decode'].detach(),
                          ...
                          normalize=True)
    np.save('w_loss.npy', sw_loss)
    np.save('w_lossv.npy', sw_lossv)
