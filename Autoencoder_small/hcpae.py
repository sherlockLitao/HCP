...
from ae.trainer_hcpae import HCPAEBatchTrainer
from torchvision import datasets, transforms
import numpy as np
import random
# for compute sw in latent and image space
from ae.trainer_swae import sliced_wasserstein_distance
import time
...
def main():
    # train args
    parser = argparse.ArgumentParser(description='HCP distance Autoencoder PyTorch MNIST Example')
    parser.add_argument('--datadir', default='./data/', help='path to dataset')
    parser.add_argument('--outdir', default='./output/hcpae/', help='directory to output images and model checkpoints')
    ...
    # create batch HCP autoencoder trainer
    trainer = HCPAEBatchTrainer(model, optimizer, distribution_fn, device=device)
    ...
    iter = 0
    ll = 20
    sw_loss = np.zeros(args.epochs)
    sw_lossv = np.zeros(args.epochs)
    end = 0
    ...
    for epoch in range(args.epochs):
        ...
        sd = np.zeros(ll)
        for l in range(ll):
            id = np.random.choice(10**4,1000,replace=False)
            sd[l] = np.log(sliced_wasserstein_distance(torch.from_numpy(test_encode[id,:]),
                                distribution_fn=distribution_fn))/2
        sw_loss[epoch] = np.mean(sd)
        sw_lossv[epoch] = np.std(sd)
    ...
    np.save('hcp_loss.npy', sw_loss)
    np.save('hcp_lossv.npy', sw_lossv)
